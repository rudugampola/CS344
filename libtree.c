...
tree_print_recurse(struct fileinfo finfo)
{
  ...
  for (size_t i = 0; i < file_count; ++i) {
    struct fileinfo finfo = {file_list[i].path, file_list[i].st};
...
print_path_info(struct fileinfo finfo)
{
  ...
  if (opts.user) {
    /*  Hint: getpwuid(3) */
    struct passwd *pwd;
    pwd = getpwuid(finfo.st.st_uid);
    if (printf("%c%s", sep, pwd->pw_name) < 0)
      goto exit; /* TODO */
    sep = ' ';
  }
  if (opts.group) {
